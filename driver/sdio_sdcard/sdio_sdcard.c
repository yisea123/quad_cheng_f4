...
SD_Error SD_PowerON(void)
{
	...
	uint32_t SDType=SD_STD_CAPACITY;
	 ...
	 /*初始化时的时钟不能大于400KHz*/ 
	SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV;							//SDIOCLK = 48MHz, SDIO_CK = HCLK/(118 + 2) = 400 KHz */
	SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising;
	SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;  				// 禁止旁路，使用分频
...
//===============================================================================================	
	...
	{																  
		...
		while((!validvoltage)&&(count<SD_MAX_VOLT_TRIAL))		//！等待上电完成
		{	   										   
			SDIO_CmdInitStructure.SDIO_Argument = 0x00;//发送CMD55,短响应
...
uint8_t SD_ReadDisk(uint8_t*buf,uint32_t sector,uint8_t cnt)
{
	...
	uint8_t n;
	lsector<<=9;	//扇区地址
	if((uint32_t)buf%4!=0)	//buf地址没有4字节对其
	{
	 	for(n=0;n<cnt;n++)
		{
